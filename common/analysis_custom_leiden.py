...
def evaluate(method_name, func, is_cdlib=False, *args, **kwargs):
        ...
        if hasattr(result, "modularity"):
            ...
            modularity = nx.community.modularity(args[0], communities)
...
def network_analysis(graph, file, sep = '\t', directed = False):
    ...
    #results.append(evaluate("Leiden", leiden, False, G))
    results.append(evaluate("Leiden", leiden_custom, False, G, weight=None, resolution=1.0))
...
def nx_to_igraph(G):
    ...
    edges = list(H.edges())
    g = ig.Graph(edges=edges, directed=True)  # ali directed=True, če je graf usmerjen
    return g, reverse_mapping

def leiden_custom(G, weight=None, resolution=1.0):
    import igraph as ig
    import leidenalg
    ...
    # Preslikava ID-jev (če ni ohranjena)
    nx_nodes = list(G.nodes())

    # Če obstajajo uteži
    if weight and nx.get_edge_attributes(G, weight):
    ...
    # Pretvori nazaj v node ID-je NetworkX grafa
    communities = [[nx_nodes[node] for node in community] for community in partition]

    return type("CDResult", (), {
